...
class DashboardController extends Controller
{
    ...
    public function storeVisit(Request $request)
    {
        $request->validate([
            ...
            'visit_type_label' => 'nullable|in:travel_agent,freelance,no_guide',
    ...
    public function updateVisit(Request $request, \App\Models\PartnerVisit $visit)
    {
        ...
        $vehicleTypes = $request->vehicle_types ?? [];
$plateNumbers = $request->plate_numbers ?? [];

$visit->vehicles()->delete();

foreach ($plateNumbers as $index => $plate) {
    if (filled($plate) || filled($vehicleTypes[$index] ?? null)) {
        \App\Models\CommissionVehicle::create([
            'partner_visit_id' => $visit->id,
            'plate_number' => $plate,
