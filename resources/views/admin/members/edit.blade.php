...
<div class="max-w-lg mx-auto">
    <div class="flex items-center gap-3 mb-5">
        ...
        <h2 class="text-xl font-bold text-slate-800">Edit Member</h2>
    </div>
        ...
        <div class="card p-5">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $member->phone) }}" required class="input-field">
                    @error('phone') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="label">Email</label>
                    <input type="email" name="email" value="{{ old('email', $member->email) }}" class="input-field" placeholder="email@example.com">
                </div>
                <div class="flex items-center gap-3 pt-1">
                    <input type="checkbox" name="is_active" id="is_active" value="1"
                        {{ $member->is_active ? 'checked' : '' }}
                        class="rounded border-slate-300 text-blue-500 focus:ring-blue-400">
                    <label for="is_active" class="text-sm text-slate-600 font-medium">Member Aktif</label>
                </div>
            </div>
        </div>
