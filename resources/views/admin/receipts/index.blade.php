    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr>
                    <th class="text-left py-3 px-5 text-xs font-bold text-slate-400 uppercase tracking-wide">Kode Struk</th>
                    ...
                    <th class="text-left py-3 px-5 text-xs font-bold text-slate-400 uppercase tracking-wide">Status Cetak</th>
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-blue-50/20 transition">
                    <td class="py-3 px-5">
                        <code class="text-xs bg-slate-100 px-2 py-1 rounded-lg text-blue-600 font-mono font-bold">{{ $trx->invoice_number }}</code>
                    </td>
                    ...
                    <td class="py-3 px-5">
                        <div class="text-xs font-semibold text-slate-700">
                            @if($trx->member)
                                {{ $trx->member->name }}
                                <span class="px-1.5 py-0.5 rounded bg-amber-100 text-amber-600 text-xs ml-1">Member</span>
                    ...
                    <td class="py-3 px-5">
                        @if($trx->is_printed)
                            <span class="px-2 py-1 rounded-lg bg-emerald-100 text-emerald-600 text-xs font-bold flex items-center gap-1 w-fit">
                        ...
                        @else
                            <span class="px-2 py-1 rounded-lg bg-amber-100 text-amber-600 text-xs font-bold flex items-center gap-1 w-fit">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3 h-3">
                    ...
                    <td class="py-3 px-5">
                        <div class="flex items-center gap-1.5">
                            ...
                            <a href="{{ route('kasir.receipt', $trx) }}" target="_blank"
                                class="p-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 transition" title="Lihat Struk">
...
<form method="POST" action="{{ route('admin.receipts.print', $trx) }}">
    @csrf
    <button type="submit"
        onclick="var w = window.open('{{ route('kasir.receipt', $trx) }}?print=1', '_blank');"
        class="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 transition" title="Print">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
