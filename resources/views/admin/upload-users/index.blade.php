@extends('layouts.app')
@section('title', 'Akun Upload Produk')
...
<div class="flex items-center justify-between mb-5">
    <div>
        <h2 class="text-xl font-bold text-slate-800">Akun Upload Produk</h2>
        <p class="text-sm text-slate-400 mt-0.5">Kelola akses halaman upload produk</p>
    </div>
    <div class="flex items-center gap-3">
        ...
        <button onclick="document.getElementById('create-upload-user-modal').classList.remove('hidden')"
...
<div class="card p-4 mb-5 flex items-center gap-3 bg-blue-50 border-blue-100">
    ...
    <div>
        <p class="text-xs font-bold text-blue-700">URL Halaman Upload</p>
        <p class="text-sm text-blue-600 font-mono">{{ url('/upload') }}</p>
    </div>
</div>
...
<div class="card overflow-hidden">
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-50">
                ...
                <td class="py-3 px-5">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold text-sm flex-shrink-0">
                            {{ substr($uploadUser->name, 0, 1) }}
                        </div>
                        <span class="font-semibold text-slate-700">{{ $uploadUser->name }}</span>
                    </div>
                </td>
                <td class="py-3 px-5 text-xs text-slate-500">{{ $uploadUser->email }}</td>
                <td class="py-3 px-5">
                    <span class="px-2 py-1 rounded-lg text-xs font-semibold {{ $uploadUser->is_active ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500' }}">
                        {{ $uploadUser->is_active ? 'Aktif' : 'Nonaktif' }}
                    </span>
                </td>
                <td class="py-3 px-5 text-xs text-slate-400">{{ $uploadUser->created_at->format('d/m/y') }}</td>
                <td class="py-3 px-5">
                    <div class="flex items-center gap-1.5">
                        {{-- Edit --}}
                        <button onclick="document.getElementById('edit-upload-user-{{ $uploadUser->id }}').classList.remove('hidden')"
                        ...
                        {{-- Hapus --}}
                        <form method="POST" action="{{ route('admin.upload-users.destroy', $uploadUser) }}"
            ...
            {{-- Edit Modal --}}
            <div id="edit-upload-user-{{ $uploadUser->id }}" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                <div class="bg-white rounded-2xl w-full max-w-sm p-6 relative">
                    <button onclick="document.getElementById('edit-upload-user-{{ $uploadUser->id }}').classList.add('hidden')"
                    ...
                    <h2 class="text-lg font-bold text-slate-800 mb-5">Edit Akun Upload</h2>
                    <form action="{{ route('admin.upload-users.update', $uploadUser) }}" method="POST" class="space-y-4">
                        @csrf @method('PUT')
                        <div>
                            <label class="label">Nama</label>
                            <input type="text" name="name" value="{{ $uploadUser->name }}" required class="input-field">
                        </div>
                        <div>
                            <label class="label">Email</label>
                            <input type="email" name="email" value="{{ $uploadUser->email }}" required class="input-field">
                        </div>
                        <div>
            ...
            <tr>
                <td colspan="5" class="py-14 text-center text-slate-300">
                    <p class="text-sm">Belum ada akun upload</p>
                </td>
            </tr>
            @endforelse
        </tbody>
...
{{-- Create Modal --}}
<div id="create-upload-user-modal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-sm p-6 relative">
        <button onclick="document.getElementById('create-upload-user-modal').classList.add('hidden')"
            class="absolute top-4 right-4 w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 flex items-center justify-center transition">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12"/>
            </svg>
        </button>
        <h2 class="text-lg font-bold text-slate-800 mb-5">Tambah Akun Upload</h2>
        <form action="{{ route('admin.upload-users.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="label">Nama</label>
