...
@section('content')
    ...
    <div>
        ...
        <input type="text"
            ...
            class="input-field">
                ...
                {{-- 3. Guide --}}
                <div class="mb-5">
                    <div class="flex items-center gap-5 justify-start mb-3">
                    ...
                    <div class="space-y-2">
                        <template x-for="(guide, index) in selectedGuides" :key="guide.id">
                            <div
                                class="flex items-center justify-between px-4 py-3 rounded-xl border border-emerald-100 bg-emerald-50">
                                <div>
                                    <div class="font-semibold text-sm text-slate-800">
                                        <span x-text="guide.guide_code"></span> —
                                        <span x-text="guide.name"></span>
            ...
            <span class="text-xs text-slate-400 mt-0.5">
                ...
                <div x-show="visitType === 'travel_agent'" class="mb-4">
                    <div class="flex items-center justify-between mb-2 gap-3">
                        ...
                        <button type="button"
                            ...
                            class="btn-green">+ Travel Agent Baru</button>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="group_description" placeholder="Contoh: Rombongan Jawa Barat"
                            class="input-field">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="label">Nama Tour Leader <span
                                class="text-slate-300 normal-case font-normal">(opsional)</span></label>
                        <input type="text" name="tour_leader_name" placeholder="Nama tour leader"
                            class="input-field">
                    </div>
                    <div>
                        <label class="label">No HP Tour Leader <span
                                class="text-slate-300 normal-case font-normal">(opsional)</span></label>
                    ...
                    <table class="text-sm" style="min-width: 1000px;">
                        <thead>
                            <tr class="bg-slate-50 border-b border-gray-100">
                                <th class="text-left px-4 py-3 text-xs font-bold text-gray-400 uppercase tracking-wide">
                                    Kode Visit</th>
                                <th class="text-left px-4 py-3 text-xs font-bold text-gray-400 uppercase tracking-wide">
                                    Tipe</th>
                                <th class="text-left px-4 py-3 text-xs font-bold text-gray-400 uppercase tracking-wide">
                                ...
                                <tr class="border-b border-gray-50 hover:bg-blue-50/30 transition">
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        @if ($visit->visitor_nationality)
                                            <span
                                                class="text-xs font-semibold {{ $visit->visitor_nationality === 'WNA' ? 'text-red-500' : 'text-blue-500' }}">
                                    ...
                                    <div class="bg-white rounded-2xl w-full max-w-md p-6 relative">
                                        ...
                                        <form method="POST" action="{{ route('security.visits.update', $visit) }}"
                                            ...
                                            <div>
                                                ...
                                                <div id="edit-vehicles-{{ $visit->id }}" class="space-y-2">
                                            ...
                                            <div>
                                                ...
                                                <select name="visitor_nationality"
                                                    ...
                                                    <option value="">Pilih...</option>
                                                    <option value="WNI"
                                                        {{ $visit->visitor_nationality === 'WNI' ? 'selected' : '' }}>WNI
                                                    </option>
                                                    <option value="WNA"
                                                        {{ $visit->visitor_nationality === 'WNA' ? 'selected' : '' }}>WNA
                                                    </option>
                                                    <option value="Campuran"
                                                        {{ $visit->visitor_nationality === 'Campuran' ? 'selected' : '' }}>
                                                        Campuran</option>
                                                </select>
                                            </div>
                                            <div>
    ...
    <div id="partner-modal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-5xl p-6 relative" style="max-height:90vh; overflow-y:auto;">
            ...
            <h2 class="text-lg font-bold text-slate-800 mb-5">Tambah Travel Agent Baru</h2>
        ...
        <div class="bg-white rounded-2xl w-full max-w-4xl p-6 relative" style="max-height:90vh; overflow-y:auto;">
            ...
            <div class="grid grid-cols-2 gap-6">
                <div class="space-y-4">
                    ...
                    <div class="flex justify-end">
                        <button type="button" onclick="saveGuide()" class="btn-green" style="padding:10px 20px;">Simpan
                            Guide</button>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-2xl w-full max-w-4xl p-6 relative" style="max-height:90vh; overflow-y:auto;">
            ...
            <div class="grid grid-cols-2 gap-6">
                <div class="space-y-4">
                    ...
                    <div class="flex justify-end">
                        <button type="button" onclick="saveDriver()" class="btn-primary"
                            style="padding:10px 20px;">Simpan Driver</button>
                    </div>
                </div>
                <div class="h-80 overflow-y-auto border border-gray-100 rounded-xl">
                    <table class="w-full text-sm">
    ...
    <script>
        ...
        async function saveGuide() {
            ...
            if (data.success) {
                ...
                tr.innerHTML =
                    `<td class="px-4 py-3 font-bold text-emerald-600 text-xs">${data.guide.guide_code}</td><td class="px-4 py-3"><div class="font-semibold text-slate-800">${data.guide.name}</div><div class="text-xs text-gray-400">${data.guide.phone || '-'}</div></td><td class="px-4 py-3"><span class="badge-green">0x</span></td>`;
                tbody.prepend(tr);
            }
        }
...
`;
        ...
        async function savePartner() {
            ...
            if (data.success) {
                ...
                // Tambah ke dropdown travel agent
                const select = document.querySelector('select[name="partner_id"]');
                if (select && data.partner.type === 'travel_agent') {
                    const option = document.createElement('option');
                    option.value = data.partner.id;
                    option.textContent = data.partner.name;
                    select.appendChild(option);
                }

                // Tambah row ke tabel travel agent
                const tbody = document.getElementById('partner-list-tbody');
                if (tbody) {
                    const tr = document.createElement('tr');
                    tr.className = 'border-t border-gray-100 hover:bg-blue-50/40 transition';
                    ...
                    tr.innerHTML = `
                        ...
                        <td class="px-4 py-3">
                            <div class="font-semibold text-slate-800">${data.partner.name}</div>
                            <div class="text-xs text-gray-400">${data.partner.phone || '-'}</div>
                        </td>
