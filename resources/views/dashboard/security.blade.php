...
<style>
    * { font-family: 'Poppins', sans-serif; }
    ...
    .btn-blue-sm:hover { background: #2563eb; }
...
<div class="max-w-5xl mx-auto space-y-5" x-data="securityVisitSystem()">
    ...
    <button type="button"
        onclick="document.getElementById('walkin-modal').classList.remove('hidden')"
        ...
        <form action="{{ route('security.visits.store') }}" method="POST">
            ...
            <div class="mt-5">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <button type="button"
                        onclick="document.getElementById('guide-modal').classList.remove('hidden')"
                        class="btn-green">
                        + Guide Baru
                    </button>
                </div>
                ...
                <div class="relative">
                    <input type="text"
                        x-model="guideSearch"
                        @input.debounce.300ms="searchGuides()"
                        placeholder="Ketik nama atau kode guide..."
                        class="input-field"
                        autocomplete="off">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-slate-50 border-b border-gray-100">
                            <th class="text-left px-4 py-3 text-xs font-700 text-gray-400 uppercase tracking-wide">Kode Visit</th>
                            <th class="text-left px-4 py-3 text-xs font-700 text-gray-400 uppercase tracking-wide">Partner</th>
                            <th class="text-left px-4 py-3 text-xs font-700 text-gray-400 uppercase tracking-wide">Guide / Driver</th>
                            <th class="text-left px-4 py-3 text-xs font-700 text-gray-400 uppercase tracking-wide">Kendaraan</th>
                            <th class="text-left px-4 py-3 text-xs font-700 text-gray-400 uppercase tracking-wide">Sticker</th>
                            <th class="text-left px-4 py-3 text-xs font-700 text-gray-400 uppercase tracking-wide">Keterangan</th>
                            <th class="text-left px-4 py-3 text-xs font-700 text-gray-400 uppercase tracking-wide">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="border-b border-gray-50 hover:bg-blue-50/30 transition">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <div class="text-xs text-gray-400">{{ $visit->partner ? ($visit->partner->type === 'travel_agent' ? 'Travel Agent' : 'Freelance') : 'Walk-in' }}</div>
                            </td>
                            <td class="px-4 py-3">
...
@else
    <div class="space-y-0.5">
        @foreach($visit->vehicles as $v)
            <div class="text-xs text-slate-600 font-medium">{{ $v->plate_number }} <span class="text-gray-400">· {{ $v->vehicle_type }}</span></div>
        @endforeach
    </div>
@endif
                            </td>
    ...
    <div class="bg-white rounded-2xl w-full max-w-lg p-6 relative">
        ...
        <form action="{{ route('security.walkin.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="vehicle_notes" placeholder="Contoh: Toyota Avanza" required class="input-field">
            </div>
            <div>
                <label class="label">Plat Kendaraan</label>
                <input type="text" name="vehicle_description" placeholder="Contoh: DR 1234 AB" required class="input-field">
            </div>
            <div>
                <label class="label">Keterangan</label>
                <textarea name="group_description" rows="2"
                    placeholder="Contoh: Mobil biru, 4 penumpang"
                    class="input-field" style="resize:none"></textarea>
            </div>
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div>
                <form action="{{ route('security.guides.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="flex justify-end pt-1">
                        <button type="submit" class="btn-green" style="padding:10px 20px; font-size:13px;">Simpan Guide</button>
                    </div>
                </form>
            </div>
            {{-- RIGHT --}}
                ...
                <div class="h-96 overflow-y-auto border border-gray-100 rounded-xl">
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            ...
                            <tr class="border-t border-gray-100 hover:bg-blue-50/40 transition">
                                <td class="px-4 py-3 font-bold text-blue-600 text-xs">{{ $guide->guide_code }}</td>
                                <td class="px-4 py-3">
