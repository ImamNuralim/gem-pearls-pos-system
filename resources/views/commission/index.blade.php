                ...
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 border-b border-slate-100">
                        <tr>
                            ...
                            <th class="text-left px-5 py-3 text-xs font-bold uppercase tracking-wide text-slate-400">Diambil Oleh</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-blue-50/20 transition">
                            ...
                            <td class="px-5 py-3.5">
                                <form action="{{ route('commission.update-rate', $commission) }}" method="POST" class="space-y-1">
                                    @csrf
                            ...
                            <td class="px-5 py-3.5">
                                @if($commission->taken_by)
                                    ...
                                    <div class="text-xs text-slate-400">{{ $commission->taken_at?->format('d/m/y H:i') }}</div>
                                @else
...
<button onclick="document.getElementById('edit-modal-{{ $commission->id }}').classList.remove('hidden')"
    ...
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
        <path stroke-linecap="round" stroke-linejoin="round" d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0 1 10.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0 .229 2.523a1.125 1.125 0 0 1-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.318 0h1.091A2.25 2.25 0 0 0 21 15.75V9.456c0-1.081-.768-2.015-1.837-2.175a48.055 48.055 0 0 0-1.913-.247M6.34 18H5.25A2.25 2.25 0 0 1 3 15.75V9.456c0-1.081.768-2.015 1.837-2.175a48.041 48.041 0 0 1 1.913-.247m10.5 0a48.536 48.536 0 0 0-10.5 0m10.5 0V3.375c0-.621-.504-1.125-1.125-1.125h-8.25c-.621 0-1.125.504-1.125 1.125v3.659M18 10.5h.008v.008H18V10.5Zm-3 0h.008v.008H15V10.5Z"/>
                        ...
                        {{-- Paid Modal --}}
                        @if($commission->status === 'unpaid')
                        <div id="paid-modal-{{ $commission->id }}" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                            <div class="bg-white rounded-2xl w-full max-w-sm p-6 relative">
                                <button onclick="document.getElementById('paid-modal-{{ $commission->id }}').classList.add('hidden')"
                                ...
                                <p class="text-xs text-slate-400 mb-5">{{ $commission->partner->name ?? '-' }} — Rp {{ number_format($commission->commission_amount, 0, ',', '.') }}</p>
                                <form method="POST" action="{{ route('commission.paid', $commission) }}" class="space-y-4">
                                    @csrf
                                    <div>
                                        <label class="label">Nama yang Mengambil Komisi</label>
                                        <input type="text" name="taken_by" required class="input-field" placeholder="Masukkan nama lengkap">
                                    </div>
                                    <button type="submit" class="w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold transition">
                                        Konfirmasi Lunas
                                    </button>
                                </form>
                            </div>
...
<div id="edit-modal-{{ $commission->id }}" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-md p-6 relative">
        ...
        <form action="{{ route('commission.update-detail', $commission) }}" method="POST" class="space-y-4">
            @csrf
            <div>
            ...
            <form action="{{ route('commission.store') }}" method="POST" class="space-y-4">
                    ...
                    <select name="partner_visit_id" required
                        ...
                        @foreach($availableVisits as $visit)
                            <option value="{{ $visit->id }}">
                                {{ $visit->visit_code }} — {{ $visit->partner->name }} {{ \Carbon\Carbon::parse($visit->visit_date)->format('d M Y') }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div x-show="selectedVisit" x-cloak class="rounded-xl bg-slate-50 border border-slate-100 p-4 space-y-3">
                    ...
                    <div class="flex items-center justify-between pt-1 border-t border-slate-200">
                        ...
                        <p class="font-bold text-blue-600 text-base" x-text="'Rp ' + new Intl.NumberFormat('id-ID').format(selectedVisit?.total_sales ?? 0)"></p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" name="commission_rate" min="0" max="100" step="0.01" placeholder="Contoh: 10"
                ...
                <div>
                    ...
                    <input type="date" name="pickup_deadline" class="input-field" value="{{ old('pickup_deadline') }}">
                </div>
