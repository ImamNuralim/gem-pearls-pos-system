...
<head>
    ...
    <title>Data Komisi — Gem Pearls</title>
...
<body class="bg-slate-50 min-h-screen">
    ...
    {{-- Header --}}
    <div
        class="bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div class="flex items-center gap-3">
            <img src="{{ public_path('assets/gem-pearls-logo.png') }}" class="logo">
            <div>
                <h1 class="text-sm font-bold text-slate-800">Data Komisi</h1>
                <p class="text-xs text-slate-400">{{ session('commission_user_name') }}</p>
            </div>
        </div>
        <form method="POST" action="{{ route('commission.logout') }}">
            @csrf
            <button type="submit"
                class="text-xs text-red-400 font-semibold px-3 py-1.5 rounded-lg hover:bg-red-50 transition">
                Logout
            </button>
        </form>
    </div>
    ...
    <div class="max-w-6xl mx-auto px-6 py-6 space-y-5">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-bold text-slate-800">Komisi Partner</h2>
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            <div class="card p-5">
                ...
                <p class="text-2xl font-bold text-red-500 mt-1">{{ $commissions->where('status', 'unpaid')->count() }}
                </p>
            </div>
            <div class="card p-5">
                <p class="text-xs font-bold uppercase text-slate-400 tracking-wide">Sudah Dibayar</p>
                <p class="text-2xl font-bold text-emerald-500 mt-1">{{ $commissions->where('status', 'paid')->count() }}
                </p>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 border-b border-slate-100">
                        <tr>
                            <th class="text-left px-5 py-3 text-xs font-bold uppercase tracking-wide text-slate-400">
                                Tanggal</th>
                            <th class="text-left px-5 py-3 text-xs font-bold uppercase tracking-wide text-slate-400">
                                Partner</th>
                            <th class="text-left px-5 py-3 text-xs font-bold uppercase tracking-wide text-slate-400">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @forelse($commissions as $commission)
                            <tr class="hover:bg-blue-50/20 transition">
                                ...
                                <td class="px-5 py-3.5">
                                    ...
                                    @if ($commission->visit)
                                        <div class="text-xs text-blue-500 font-medium">
                                            {{ $commission->visit->visit_code }}</div>
                                    @endif
                                </td>
                                <td class="px-5 py-3.5">
                                ...
                                <td class="px-5 py-3.5">
                                    <form action="{{ route('commission.update-rate', $commission) }}" method="POST"
                                        class="space-y-1">
                                        @csrf
                                        <div class="flex items-center gap-1.5">
                                            <input type="number" name="commission_rate"
                                                value="{{ $commission->commission_rate }}" step="0.01"
                                                min="0" max="100"
                                ...
                                <td class="px-5 py-3.5">
                                    @if ($commission->taken_by)
                                        <div class="text-xs font-semibold text-slate-700">{{ $commission->taken_by }}
                                        </div>
                                ...
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center gap-1.5">
                                        {{-- Edit --}}
                                        <button
                                            onclick="document.getElementById('edit-modal-{{ $commission->id }}').classList.remove('hidden')"
                                        ...
                                        @if ($commission->status === 'unpaid')
                                            <button
                                                onclick="document.getElementById('paid-modal-{{ $commission->id }}').classList.remove('hidden')"
                                        ...
                                        @endif
                                        {{-- Delete --}}
                                        <form action="{{ route('commission.destroy', $commission) }}" method="POST"
                            ...
                            @if ($commission->status === 'unpaid')
                                <div id="paid-modal-{{ $commission->id }}"
                                    ...
                                    <div class="bg-white rounded-2xl w-full max-w-sm p-6 relative">
                                        ...
                                        <form method="POST" action="{{ route('commission.paid', $commission) }}"
                                            class="space-y-4">
                                            @csrf
                                            <div>
                            ...
                            <div id="edit-modal-{{ $commission->id }}"
                                ...
                                <div class="bg-white rounded-2xl w-full max-w-md p-6 relative">
                                    ...
                                    <form action="{{ route('commission.update-detail', $commission) }}"
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="commission_rate"
                                                value="{{ $commission->commission_rate }}" step="0.01"
                                                min="0" max="100" required class="input-field">
                                        </div>
                                        <div>
    ...
    <div id="create-commission-modal"
        class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" x-data="{ selectedVisit: null, visits: @json($availableVisits) }">
        <div class="bg-white rounded-2xl w-full max-w-lg p-6 relative" style="max-height:90vh; overflow-y:auto;">
            ...
            <form action="{{ route('commission.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="label">Kunjungan</label>
                ...
                <div x-show="selectedVisit" x-cloak
                    class="rounded-xl bg-slate-50 border border-slate-100 p-4 space-y-3">
                    ...
                    <div class="flex items-center justify-between pt-1 border-t border-slate-200">
                        ...
                        <p class="font-bold text-blue-600 text-base"
                            x-text="'Rp ' + new Intl.NumberFormat('id-ID').format(selectedVisit?.total_sales ?? 0)">
                        </p>
                    </div>
                </div>
