...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-60 bg-white border-r border-slate-100 flex flex-col shadow-sm flex-shrink-0">
        ...
        <div class="px-5 py-3 border-b border-slate-100 bg-blue-50/50">
            <p class="text-xs font-semibold text-slate-700">{{ auth()->check() ? auth()->user()->name : 'Guest Mode' }}</p>
            <span class="inline-block mt-0.5 text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 font-semibold">
        ...
        <nav class="flex-1 px-3 py-3 overflow-y-auto space-y-0.5">
            ...
            <a href="{{ route('admin.tax.index') }}" class="nav-link {{ request()->routeIs('admin.taxes*') ? 'active' : '' }}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="nav-icon">
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="nav-icon text-slate-400">
...
<a href="{{ route('admin.users.index') }}" class="nav-link {{ request()->routeIs('admin.users*') ? 'active' : '' }}">
        ...
        <div class="px-3 py-3 border-t border-slate-100">
            @if(auth()->check())
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="nav-link w-full text-red-400 hover:bg-red-50 hover:text-red-600">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="nav-icon">
    ...
    <main class="flex-1 overflow-y-auto">
        <div class="bg-white border-b border-slate-100 px-8 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
